// Android FFmpeg implementation using FFmpegKitNext
import 'dart:async';
import 'dart:io';
...
class AndroidFFmpegService implements MediaProcessingService {
  final FFmpegKitWrapper _ffmpegKit;
  bool _initialized = false;
  bool _disposed = false;

  AndroidFFmpegService(this._ffmpegKit);

  @override
  Future<void> initialize() async {
    if (_initialized) return;
    if (!PlatformUtils.canUseDartIO) {
      throw UnsupportedPlatformError('AndroidFFmpegService requires native platform');
    }
  ...
  @override
  bool get isAvailable => PlatformUtils.currentPlatform == TargetPlatform.android && _initialized;

  @override
  String get serviceName => 'FFmpegKitNext (Android)';
  ...
  void _ensureInitialized() {
    if (!_initialized) {
      throw StateError('AndroidFFmpegService not initialized. Call initialize() first.');
    }
    if (_disposed) {
      throw StateError('AndroidFFmpegService has been disposed.');
    }
  }
