// iOS FFmpeg implementation using FFmpegKitNext
import 'dart:async';
import 'dart:io';
...
class IosFFmpegService implements MediaProcessingService {
  final FFmpegKitWrapper _ffmpegKit;
  bool _initialized = false;
  bool _disposed = false;

  IosFFmpegService(this._ffmpegKit);

  @override
  Future<void> initialize() async {
    if (_initialized) return;
    if (!PlatformUtils.canUseDartIO) {
      throw UnsupportedPlatformError('IosFFmpegService requires native platform');
    }
  ...
  @override
  bool get isAvailable => PlatformUtils.currentPlatform == TargetPlatform.ios && _initialized;

  @override
  String get serviceName => 'FFmpegKitNext (iOS)';
  ...
  void _ensureInitialized() {
    if (!_initialized) {
      throw StateError('IosFFmpegService not initialized. Call initialize() first.');
    }
    if (_disposed) {
      throw StateError('IosFFmpegService has been disposed.');
    }
  }
